...
- **Code signing — Posture 3 (skip all signing at MVP).** Deliberate, time-bounded decision:
  ...
  - **(UC 26, 2026-05-10) macOS-only Posture-3 → Posture-1 upgrade.** macOS 26.x's `AppleSystemPolicy` denies exec at dyld-startup for unsigned arm64 binaries even after the user grants the Gatekeeper "open anyway" override (see `use-cases/26-fix-macos-arm64-launch-failure.md` for the `sample`-based root cause). The macOS `.dmg` is therefore upgraded to Posture 1 (Developer ID + hardened runtime + per-binary entitlements + notarization + DMG-only stapling) ahead of the original three-trigger schedule, because the failure mode is a hard launch block rather than a UX warning. Linux + Windows remain on Posture 3 — they have not had a forcing function. Full design in ADR 0011; secret inventory in README § macOS release prerequisites; new threat surface in THREATS.md § T15.
